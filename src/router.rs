...
use crate::httpresponse::{Body, HttpResponse};
                    ...
                    match route[1] {
                        ...
                        "user-agent" => {
                            ...
                            if let Some(header) = req.headers.get("User-Agent") {
                                resp.body = Some(Body::Text(header.trim().to_string()));
                            }
                            let _ = resp.send_response(stream);
                        }
                        "files" => {
                            ...
                            let exists =
                                std::path::Path::new(&format!("{}{}", dir, filename)).exists();
                            if exists {
                                let b = std::fs::read_to_string(&format!("{}{}", dir, filename))
                                    .unwrap();
                                ...
                                let resp: HttpResponse =
                                    HttpResponse::new("200", h, Some(Body::Text(b)));
                                let _ = resp.send_response(stream);
                            } else {
                        ...
                        "echo" => {
                            ...
                            let mut resp: HttpResponse = HttpResponse::new(
                                "200",
                                None,
                                Some(Body::Text(route[2].to_string())),
                            );
                            if accept_encoding.is_some() {
                                let mut encodings = accept_encoding.unwrap().split(",");
                                if let Some(h) =
                                    encodings.find(|e| e.to_lowercase().trim() == "gzip")
                                {
                                    ...
                                    encoder
                                        .write_all(route[2].as_bytes())
                                        .expect("Failed to write");
                                    let compressed_data =
                                        encoder.finish().expect("Failed to finish");

                                    resp.body = Some(Body::Binary(compressed_data));
                                }
                            }
                            let _ = resp.send_response(stream);
                        }
                ...
                httprequest::Resource::Path(s) => {
                    ...
                    match route[1] {
                        "files" => {
                            ...
                            let mut file =
                                std::fs::File::create(&format!("{}{}", dir, filename)).unwrap();
